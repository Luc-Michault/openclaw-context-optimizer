...
**Opt-in:** set `suggestOnLargeRead: true` for a narrow `before_tool_call` hook. For read-like tools (default `read`), files **≥ `maxFileBytes`** trigger **`emitLargeReadSuggestion`**: structured object (**`schemaVersion` `1.0.0`**, same fields as **`advise`** incl. **`confidenceScore`**, **`worthReadingExactlyReasons`**, **`isBinaryArtifact`**, **`pathIssue`**), optional **`onSuggestion(suggestion, { toolName, params })`** when the plugin is registered **in code** (not from JSON), and **`renderSuggestionLogLine`** unless **`logSuggestions: false`** or **`silent: true`**. No tool rewriting. **`matchers`**: path substring or light glob; **`extensions`**: allowlist when set. **`suggestDryRunVerbose: true`**: logs a JSON **`traceLargeReadSuggestion`** line on each candidate read (filter gate + advise summary). Use **`require('openclaw-context-optimizer/suggest')`** or **`require('./suggest')`** for **`buildLargeReadSuggestion`**, **`traceLargeReadSuggestion`**, **`emitLargeReadSuggestion`** without the hook.
